...
export default function VeillePage() {
  ...
  useEffect(() => {
    ...
    const queryLabel = GDELT_QUERIES[tenantId].label
    setLoading(true)
    setArticles(null)
    setVolume(null)
    ...
    fetchGdeltArticles(tenantId, ctrl.signal)
      .then((list) => {
        ...
        const detection = detectNegativeCampaign(list, queryLabel, tenantName)
        ...
        if (result === 'created' && detection) {
          ...
          toastFn(`Alerte temps réel — ${detection.title}`, {
            description: 'Créée automatiquement sur la page Alertes & Incidents.',
          })
        }
      })
      .catch((err: unknown) => {
        if (ctrl.signal.aborted) return
        setLoading(false)
        toast.error('Données temps réel indisponibles — affichage des données de démonstration', {
          description: isGdeltRateLimit(err)
            ? 'GDELT limite les requêtes à 1 toutes les 5 secondes — réessayez dans un instant.'
            : 'Le point d’accès GDELT ne répond pas (réseau ou proxy).',
        })
        setMode('demo')
      })
    ...
    timer = window.setTimeout(() => {
      fetchGdeltVolume(tenantId, ctrl.signal)
        .then((points) => {
          ...
          const detection = detectVolumeSpikeAlert(points, queryLabel, tenantName)
          const result = reportDetection(tenantId, tenantName, 'pic', detection)
          if (result === 'created' && detection) {
            toast.warning(`Alerte temps réel — ${detection.title}`, {
              description: 'Créée automatiquement sur la page Alertes & Incidents.',
            })
          }
        })
  ...
  }, [mode, tenantId, reloadTick, tenant.meta.name, reportDetection])
  ...
  const handleGenerateBrief = async () => {
    ...
    if (llmConfig.apiKey.trim()) {
      try {
        const brief = await generateLlmBrief(articles, volume, queryLabel, llmConfig)
        setLiveBrief(brief)
        setBriefLoading(false)
        return
      } catch {
        toast.error('API LLM indisponible — bascule sur le générateur heuristique', {
          description: 'Vérifiez la clé, l’URL de base ou réessayez (délai maximum : 10 s).',
        })
      }
    }
    // Délai volontaire pour rendre l'état de génération perceptible
    await new Promise((resolve) => setTimeout(resolve, 800))
    setLiveBrief(generateHeuristicBrief(articles, volume, queryLabel))
    setBriefLoading(false)
  }
  ...
  const distribution = useMemo(() => {
    ...
    return (Object.keys(counts) as Sentiment[]).map((k) => ({ name: SENTIMENT_LABEL[k], key: k, value: counts[k] }))
  }, [mode, articleRows, tenant])
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <PageHeader
        title="Veille & Réputation"
        description={
          mode === 'live'
            ? `Écoute en temps réel — requête GDELT : ${GDELT_QUERIES[tenantId].label}.`
            : 'Écoute des sources publiques — presse, réseaux sociaux, forums et canaux Telegram. Fenêtre : 13 – 19 juillet 2026.'
        }
        actions={
          <div className="flex rounded-md border border-zinc-700 bg-zinc-900 p-0.5">
            ...
            <button
              ...
              title={crisis.active ? 'Indisponible pendant la simulation de crise' : 'Interroger l’API GDELT en direct'}
              className={cn(
                'rounded px-3 py-1.5 text-xs font-medium transition-colors',
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 rounded-md border border-emerald-500/30 bg-emerald-500/5 px-4 py-3">
          ...
          <p className="min-w-0 flex-1 text-sm text-zinc-200">
            {tenantLiveAlerts.length === 1
              ? 'Alerte créée sur la page Alertes'
              : `${tenantLiveAlerts.length} alertes créées sur la page Alertes`}
            {' — '}
          ...
          <Link to="/alertes" className="flex items-center gap-1 text-xs font-medium text-emerald-400 hover:text-emerald-300">
            Voir les alertes <ArrowRight className="h-3 w-3" />
          </Link>
        </div>
      )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <CardTitle className="text-sm font-medium text-zinc-200">
                  {mode === 'live' ? 'Articles en temps réel' : 'Flux de mentions'}
                </CardTitle>
                {mode === 'live' && (
              ...
              <div className="flex items-center gap-3">
                  ...
                  <>
                    {lastSync && (
                      <span className="text-[11px] tabular-nums text-zinc-500">
                        Dernière synchro {lastSync.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    )}
                    <Button
                      variant="outline"
                ...
                <Tabs value={filter} onValueChange={(v) => setFilter(v as Filter)}>
                  <TabsList className="bg-zinc-800/80">
                    ...
                    <TabsTrigger value="negatif">Négatives</TabsTrigger>
                    <TabsTrigger value="neutre">Neutres</TabsTrigger>
                    <TabsTrigger value="positif">Positives</TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
            </div>
            ...
            {mode === 'demo' && (
              <>
                {filteredMentions.length === 0 && (
                  <p className="py-6 text-center text-sm text-zinc-500">Aucune mention dans cette catégorie.</p>
                )}
                  ...
                  <article key={m.id} className="rounded-md border border-zinc-800 bg-zinc-950/50 p-3.5">
                    ...
                    <div className="mt-2.5 flex items-center gap-3">
                      ...
                      <span className="flex items-center gap-1 text-xs text-zinc-500">
                        <Eye className="h-3.5 w-3.5" /> {formatReach(m.reach)} contacts
                      </span>
                    </div>
                  </article>
                ))}
            ...
            {mode === 'live' && !loading && (
              <>
                {filteredArticles.length === 0 && (
                  <p className="py-6 text-center text-sm text-zinc-500">Aucun article dans cette catégorie.</p>
                )}
        ...
        <div className="space-y-4">
          <Card className="bg-zinc-900/70">
            <CardHeader className="pb-0">
              <div className="flex items-center justify-between gap-2">
                <CardTitle className="text-sm font-medium text-zinc-200">Répartition du sentiment</CardTitle>
                {mode === 'live' && (
                  <Badge variant="outline" className="border-sky-400/40 bg-sky-400/10 text-[10px] font-normal text-sky-400">
                    Analyse automatique (heuristique)
                  </Badge>
                )}
              </div>
              <p className="text-xs text-zinc-500">
                {mode === 'live' ? 'Titres des articles — fenêtre 7 jours' : 'Mentions échantillonnées — 24 h'}
              </p>
            </CardHeader>
            <CardContent>
              <div className="relative h-52">
                ...
                <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[11px] text-zinc-500">{mode === 'live' ? 'articles' : 'mentions'}</span>
                </div>
              </div>
          ...
          <Card className="bg-zinc-900/70">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between gap-2">
                <CardTitle className="text-sm font-medium text-zinc-200">Principales sources</CardTitle>
              ...
              {tenant.topSources.map((s) => (
                <div key={s.name}>
                  <div className="mb-1 flex items-center gap-2 text-sm">
                    ...
                    <span className="text-xs tabular-nums text-zinc-500">{s.mentions} ment.</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-zinc-800">
                      <div className="h-full rounded-full bg-red-500/80" style={{ width: `${s.negativity}%` }} />
                    </div>
                    <span className="w-16 text-right text-[11px] text-zinc-500">{s.negativity} % nég.</span>
                  </div>
                </div>
              ))}
            </CardContent>
      ...
      {mode === 'live' && (
        <Card className="bg-zinc-900/70">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-zinc-200">Volume de mentions — 7 jours</CardTitle>
            <p className="text-xs text-zinc-500">Nombre d’articles par jour · source GDELT (timelinevolinfo)</p>
          </CardHeader>
          <CardContent className="h-64">
            {volumeError && (
              <div className="flex h-full items-center justify-center rounded-md border border-dashed border-zinc-700 text-sm text-zinc-500">
                Série temporelle indisponible (limitation de débit GDELT) — elle se rechargera à la prochaine actualisation.
              </div>
            )}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={volume} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                  ...
                  <Tooltip
                    contentStyle={{ background: '#18181b', border: '1px solid #3f3f46', borderRadius: 8, fontSize: 12 }}
                    itemStyle={{ color: '#e4e4e7' }}
                    formatter={(value) => [`${value} articles`, 'Volume']}
                  />
      ...
      <Card className="bg-zinc-900/70">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between gap-2">
            <CardTitle className="text-sm font-medium text-zinc-200">Cartographie de risque — sujets × tonalité</CardTitle>
            {mode === 'live' && <DemoDataBadge />}
          </div>
          <p className="text-xs text-zinc-500">Volume de mentions par sujet et par tonalité, niveau de risque agrégé (portée × négativité)</p>
        </CardHeader>
        <CardContent className="overflow-x-auto">
          <table className="w-full min-w-[560px] border-separate border-spacing-1 text-sm">
            <thead>
              <tr>
                <th className="px-3 py-1.5 text-left text-xs font-medium uppercase tracking-wide text-zinc-500">Sujet</th>
                <th className="px-3 py-1.5 text-center text-xs font-medium uppercase tracking-wide text-emerald-500/80">Positif</th>
                <th className="px-3 py-1.5 text-center text-xs font-medium uppercase tracking-wide text-zinc-400">Neutre</th>
                <th className="px-3 py-1.5 text-center text-xs font-medium uppercase tracking-wide text-red-500/80">Négatif</th>
                <th className="px-3 py-1.5 text-right text-xs font-medium uppercase tracking-wide text-zinc-500">Risque</th>
              </tr>
            </thead>
            <tbody>
      ...
      <Card id="brief" className="border-sky-400/25 bg-zinc-900/70">
        <CardHeader className="border-b border-zinc-800 pb-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <CardTitle className="text-base font-semibold text-zinc-50">Brief quotidien IA</CardTitle>
              <p className="text-xs text-zinc-500">
                {mode === 'live'
                  ? `Génération à la demande à partir des titres GDELT — ${GDELT_QUERIES[tenantId].label}`
            ...
            {mode === 'live' && liveBrief && (
              <>
                <Badge variant="outline" className="border-emerald-500/40 bg-emerald-500/10 text-[10px] font-normal text-emerald-400">
                  Généré automatiquement à partir de {liveBrief.articleCount} articles réels —{' '}
                  {liveBrief.generatedAt.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                </Badge>
                <Badge
                  variant="outline"
                  className={cn(
                ...
                >
                  {liveBrief.engine === 'llm' ? 'IA (API externe)' : 'heuristique'}
                </Badge>
              </>
            )}
            {mode === 'demo' && (
              <span className="rounded-full border border-zinc-700 bg-zinc-950 px-3 py-1 text-[11px] text-zinc-400">
                Généré le {tenant.brief.generatedAt} · moteur d’analyse Bastion · validé par l’équipe veille
              </span>
            )}
          </div>
        </CardHeader>
        <CardContent className="grid grid-cols-1 gap-6 pt-5 lg:grid-cols-2">
          ...
          <section className="lg:col-span-2">
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-sky-400">Synthèse</h3>
          ...
          <section>
            <h3 className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-sky-400">
              <Signal className="h-3.5 w-3.5" /> Signaux faibles
            </h3>
          ...
          <section>
            <h3 className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-sky-400">
              <AlertTriangle className="h-3.5 w-3.5" /> Incidents détectés
            </h3>
            <ul className="space-y-2">
            ...
            <div className="space-y-4 lg:col-span-2">
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  ...
                  {liveBrief ? 'Régénérer le brief' : 'Générer le brief du jour'}
                </Button>
                {briefLoading && <span className="text-xs text-zinc-500">Génération en cours…</span>}
                {!briefLoading && !loading && (!articles || articles.length === 0) && (
                  <span className="text-xs text-zinc-500">En attente d’articles réels…</span>
                )}
              </div>
                ...
                <CollapsibleContent className="space-y-3 border-t border-zinc-800 px-3.5 py-3">
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="space-y-1.5">
                      <Label htmlFor="llm-base-url" className="text-[11px] text-zinc-500">URL de base</Label>
                      <Input
                        id="llm-base-url"
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="llm-api-key" className="text-[11px] text-zinc-500">Clé API</Label>
                      <Input
                        id="llm-api-key"
                        type="password"
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="llm-model" className="text-[11px] text-zinc-500">Modèle</Label>
                      <Input
                        id="llm-model"
                  ...
                  <p className="text-[11px] text-amber-500/80">Prototype — la clé reste dans ce navigateur.</p>
                  {llmConfig.apiKey.trim() && (
                    <p className="text-[11px] text-emerald-500/80">
                      Clé configurée : la prochaine génération utilisera l’API externe ({llmConfig.model || DEFAULT_LLM_CONFIG.model}),
                      avec bascule automatique sur l’heuristique en cas d’erreur.
                    </p>
                  )}
                </CollapsibleContent>
              </Collapsible>
              ...
              {!briefLoading && !liveBrief && (
                <div className="rounded-md border border-dashed border-zinc-700 px-4 py-6 text-center text-sm text-zinc-500">
                  {articles && articles.length > 0
                    ? 'Aucun brief généré pour le moment — lancez la génération à partir des articles réels récupérés.'
                    : 'Le brief pourra être généré dès que les articles GDELT seront chargés.'}
                </div>
              )}
